        ...
        for w, c in sorted(count.items(), key=lambda x: (-x[1], x[0])):
            if c >= freq_min:
                if w not in vocab:
                    vocab[w] = len(vocab)
        update_vocab = False
        print("made frequency-based vocab")
...
def get_other_words_and_vocab(file_path):
    vocab = {'<eos>': 0, '<unk>': 1}
    train, vocab = tokenize_text(
        file_path,
        vocab, update_vocab=True, freq_min=3)
    sp1, sp2 = int(len(train) * 0.05), int(len(train) * 0.95)
    valid, train, test = train[:sp1], train[sp1:sp2], train[sp2:]

    json.dump(vocab, open(file_path + '.vocab', 'w'))
    return train, valid, test, vocab
